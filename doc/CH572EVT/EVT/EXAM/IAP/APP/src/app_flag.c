...
void SwitchImageFlag(uint8_t new_flag)
{
    ...
    /* 读取第一块 */
    FLASH_ROM_READ(IAP_FLAG_DATAFLASH_ADD, (PUINT32) &imgFlag, 4);
    if (imgFlag.ImageFlag != new_flag)
    {
        /* 擦除第一块 */
        PRINT("%d\n",FLASH_ROM_ERASE(IAP_FLAG_DATAFLASH_ADD, 4096));

        /* 更新Image信息 */
        imgFlag.ImageFlag = new_flag;

        /* 编程DataFlash */
        FLASH_ROM_WRITE(IAP_FLAG_DATAFLASH_ADD, (PUINT32) &imgFlag, 4);
    }
}
